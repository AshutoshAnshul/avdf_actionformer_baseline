...
def load_gt_from_json_avdf1m(json_file):
    ...
    for meta in json_db:
        if ('fake_segments' in meta) and (len(meta['fake_segments']) > 0):
            ...
            vids+=[str(meta['file']).strip()]*num_acts

            for event in valid_acts:
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.split = split
        self.ground_truth = load_gt_from_json_avdf1m(ant_file)
...
##----------------------------------------------------------------------------------------------------##
# AR and AP value calculation according to https://github.com/ControlNet/LAV-DF/blob/master/metrics.py
...
class AP:
    ...
    def __call__(self, metadata: list, proposals_dict: dict) -> dict:
        ...
        for iou_threshold in self.iou_thresholds:
            ...
            for meta in tqdm(metadata):
                proposals = torch.tensor(proposals_dict[meta.id])
                # labels = torch.tensor(meta.fake_periods)
                labels = torch.tensor(meta.segments)
...
class AR:
    ...
    def __call__(self, metadata: list, proposals_dict: dict) -> dict:
        ...
        for i, meta in enumerate(tqdm(metadata)):
            ...
            # labels = torch.tensor(meta.fake_periods)
            labels = torch.tensor(meta.fake_segments)
            values[i] = self.get_values(self.iou_thresholds, proposals, labels)
